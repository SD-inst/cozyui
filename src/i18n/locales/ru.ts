import { statusEnum } from '../../redux/progress';

const translation = {
    controls: {
        width: 'ширина',
        height: 'высота',
        aspect_ratio: 'соотношение сторон',
        max_width: 'макс. ширина',
        max_height: 'макс. высота',
        length: 'длительность',
        steps: 'шаги',
        cfg: 'CFG',
        guidance: 'гайданс',
        flow_shift: 'сдвиг потока',
        wave_speed: 'кэш WaveSpeed',
        wave_speed_maxhit: 'макс. попаданий WaveSpeed',
        wave_speed_start: 'начало WaveSpeed',
        wave_speed_end: 'конец WaveSpeed',
        size: 'макс. размер',
        tea_cache: 'Tea Cache',
        enhance_video: 'вес Enhance-a-Video',
        aug_strength: 'добавление шума',
        latent_strength: 'сила латента',
        stg: 'STG',
        stg_rescale: 'STG рескейл',
        stg_mode: 'режим STG',
        compression: 'сжатие',
        neg_prompt: 'негативный промпт',
        scheduler: 'планировщик',
        sampler: 'сэмплер',
        seed: 'сид',
        prompt: 'промпт',
        model: 'модель',
        video_vae: 'video VAE',
        suffix: 'суффикс после описания',
        llm: 'LLM для описания',
        attention: 'аттеншен',
        generate: 'Сгенерировать',
        describe: 'Описать',
        interrupt: 'Прервать',
        advanced_parameters: 'Расширенные параметры',
        video: 'Видео',
        audio: 'Аудио',
        first_frame: 'Первый кадр',
        last_frame: 'Последний кадр',
        no_audio: 'без аудио',
        audio_source: 'источник аудио',
        audio_file: 'аудио-файл',
        download: 'Скачать',
        download_png: 'Скачать PNG',
        download_jpg: 'Скачать JPG',
        blocks_to_swap: 'Выгружать блоков',
        vace_blocks_to_swap: 'Выгружать блоков VACE',
        single_blocks: 'Единые блоки',
        double_blocks: 'Двойные блоки',
        offload_txt_in: 'Выгружать txt in',
        offload_img_in: 'Выгружать img in',
        offload_txt_emb: 'Выгружать txt emb',
        offload_img_emb: 'Выгружать img emb',
        block_swap: 'включить выгрузку блоков (экономит VRAM, замедляет)',
        lora: 'LoRA',
        lora_reload: 'Перезагрузить список лор',
        randomize_seed: 'Случайный сид',
        history: 'История',
        tab: 'Вкладка',
        history_empty: 'Пока пусто',
        filter: 'Фильтр',
        settings: 'Настройки',
        ok: 'ОК',
        cancel: 'Отмена',
        close: 'Закрыть',
        queued: 'Очередь: %{queue}',
        compare_arr_with_this: 'Сравнить <--> с этим',
        compare_prev: 'Сравнить с предыдущим',
        compare_this_with: 'Сравнить это с...',
        reset_comparison: 'Сбросить сравнение',
        show_params: 'Показать параметры',
        generation_params: 'Параметры генерации',
        difference: 'Различия',
        image: 'изображение',
        image_end: 'конечное изображение',
        drop_files_here: 'Перетащите файлы сюда...',
        drop_files_desc: 'Перетащите файлы сюда или кликните для выбора файлов',
        drop_to_add: 'Отпустите, чтобы добавить',
        merge_type: 'тип слияния',
        merge_type_single: 'Только единые блоки',
        merge_type_double: 'Только двойные блоки',
        merge_type_full: 'Полностью',
        strength: 'сила',
        model_page: 'оригинальная модель',
        model_page_civitai_red: 'civitai.red',
        change_lora_merge_params: 'Изменить параметры лоры «%{name}»',
        delete_result: 'Удалить результат',
        confirm_delete_result: 'Вы уверены, что хотите удалить этот результат?',
        send_result: 'Отправить результат',
        unknown_media: 'Неизвестный тип медии',
        duration: '[%{sec} с]',
        pin: 'Закрепить',
        unpin: 'Открепить',
        pinned: 'Закреплённое',
        unpin_title: 'Открепить результат',
        unpin_confirm:
            'Вы уверены, что хотите открепить этот результат? Его станет возможно удалить через очистку истории.',
        reset: 'Сброс',
        remove: 'Удалить',
        replace: 'Заменить',
        crop: 'Обрезать',
        rotate_cw: 'Повернуть на 90° по часовой стрелке',
        rotate_ccw: 'Повернуть на 90° против часовой стрелки',
        skip: 'Пропустить',
        skip_chat: 'Пропустить для чата',
        reset_form: 'Сброс формы',
        save_session: 'Сохранить как сессию',
        confirm_reset: 'Подтвердите сброс',
        confirm_reset_content:
            'Вы уверены, что хотите сбросить поля на этой вкладке на значения по умолчанию?',
        reset_array: 'Сброс массива',
        confirm_reset_array: 'Подтвердите сброс',
        confirm_reset_array_content:
            'Вы уверены, что хотите удалить все элементы этого массива?',
        import_history: 'Импорт истории',
        export_history: 'Экспорт истории',
        export: 'Экспорт',
        import: 'Импорт',
        export_item: 'Экспорт',
        export_import: 'Экспорт / импорт',
        please_wait: 'Пожалуйста, подождите...',
        denoise: 'денойз',
        upscale: 'увеличение',
        inpainting: 'инпейнтинг',
        inpaint_crop: 'инпейнт кроп',
        inpaint_crop_settings: 'Настройки инпейнт кроп',
        mask_expand_pixels: 'расширение маски',
        mask_blend_pixels: 'слияние маски',
        context_expand_factor: 'фактор контекста',
        target_width: 'целевая ширина',
        target_height: 'целевая высота',
        output_padding: 'отступ',
        mask_editor: {
            title: 'Редактор маски',
            no_image: 'Нет изображения',
            toolbar: 'Редактор маски',
            expand_toolbar: 'Развернуть панель',
            collapse_toolbar: 'Свернуть панель',
            brush_size: 'Размер кисти',
            mask_color: 'Цвет маски',
            mask_opacity: 'Прозрачность маски',
            reset_mask: 'Сбросить маску',
            undo: 'Отменить',
            redo: 'Повторить',
            fullscreen: 'Полноэкранный режим',
            exit_fullscreen: 'Выйти из полноэкранного режима',
            erase_mode: 'Режим стирания',
            draw_mode: 'Режим рисования',
        },
        send_to_upscale: 'Отправить на увеличение',
        allow_upscale: 'разрешить увеличение',
        clip_model: 'модель CLIP LLM',
        tab_json_diff: 'Параметры',
        tab_media_diff: 'Медиа',
        tab_timings_diff: 'Тайминги',
        timings_other: 'прочее',
        no_timings: 'Тайминги не записаны',
        node_timings: 'Тайминги нод',
        phase: 'фаза',
        per_step: 'с/шаг',
        total_time: 'общее время',
        virtual_vram: 'виртуальная VRAM',
        riflex: 'RIFLEx',
        compile_model: 'скомпилировать модель',
        length_s: 'длина в секундах',
        window_size: 'размер сегмента (в латентах)',
        rel_thresh: 'порог Tea Cache',
        batch_size: 'размер батча',
        rescale_cfg: 'масштаб CFG',
        tabs: 'Вкладки',
        tile_width: 'ширина тайла',
        tile_height: 'высота тайла',
        upscale_model: 'модель увеличения',
        padding: 'бордюр тайла',
        mask_blur: 'размытие маски',
        self_forcing_lora: 'включить ускорение Self Forcing (только 14B)',
        start_strength: 'сила начального изображения',
        end_strength: 'сила конечного изображения',
        use_end_image: 'использовать конечное изображение',
        nag: 'включить NAG',
        nag_scale: 'NAG масштаб',
        nag_tau: 'NAG тау',
        nag_alpha: 'NAG альфа',
        nag_sigma_end: 'NAG последняя сигма',
        sigma_shift_video: 'Sigma Shift Video',
        sigma_shift_audio: 'Sigma Shift Audio',
        spectrum: 'Spectrum',
        spectrum_enabled: 'Включить Spectrum',
        spectrum_blend_weight: 'Spectrum Blend Weight',
        spectrum_flex_window: 'Spectrum Flex Window',
        spectrum_degree: 'Spectrum Degree',
        spectrum_warmup_steps: 'Spectrum Warmup Steps',
        spectrum_bootstrap_first_forecast: 'Spectrum Bootstrap First Forecast',
        turbo: 'Турбо',
        pdd: 'PDD',
        turbo_lora: 'Турбо LoRA',
        turbo_lora_strength: 'Сила',
        latent_upscale: 'Латентный апскейл',
        latent_upscale_megapixels: 'мегапиксели апскейла',
        latent_upscale_steps: 'шаги апскейла',
        latent_upscale_sampler: 'сэмплер апскейла',
        latent_upscale_seed: 'сид апскейла',
        sparse_attention: 'Разрежённое внимание',
        sparse_attention_video_budget: 'Бюджет видео (разрежённое внимание)',
        sparse_attention_denser_early_late_steps: 'Плотные ранние/поздние шаги',
        send_back: 'Отправить назад',
        send_to_i2i: 'Отправить в I2I',
        append_image: 'Добавить изображение',
        second_image: 'Второе изображение',
        third_image: 'Третье изображение',
        latents_loaded: 'Используются латенты, не изображение (выше качество)',
        full_steps: 'полных шагов',
        refine_steps: 'улучшающих шагов',
        max_full_steps: 'макс. полных шагов',
        audio_toggle: 'использовать образец аудио',
        audio_toggle_C: 'использовать собеседника C',
        audio_toggle_D: 'использовать собеседника D',
        audio_toggle_3: 'использовать собеседника 3',
        audio_toggle_4: 'использовать собеседника 4',
        audio_input_A: 'загрузить аудио для собеседника A',
        audio_input_B: 'загрузить аудио для собеседника B',
        audio_input_C: 'загрузить аудио для собеседника C',
        audio_input_D: 'загрузить аудио для собеседника D',
        audio_input_1: 'загрузить аудио для собеседника 1',
        audio_input_2: 'загрузить аудио для собеседника 2',
        audio_input_3: 'загрузить аудио для собеседника 3',
        audio_input_4: 'загрузить аудио для собеседника 4',
        audio_input: 'загрузить аудио',
        exaggeration: 'выразительность',
        temperature: 'температура',
        send_to: 'Отправить в...',
        reuse_threshold: 'порог кэширования',
        start_percent: 'начало кэширования',
        end_percent: 'конец кэширования',
        cache_device: 'устройство для кэша TeaCache',
        loop: 'цикл',
        shift_skip: 'сдвиг',
        loop_start_percent: 'начало цикла',
        loop_end_percent: 'конец цикла',
        color_correction: 'алгоритм цветокоррекции',
        top_p: 'топ P',
        use_sampling: 'включить сэмплирование',
        audio_cfg_scale: 'аудио CFG',
        audio_scale: 'сила аудио',
        fps: 'кадров/сек',
        model_it: 'модель InfiniteTalk',
        type: 'Тип контента',
        type_any: 'Любой',
        type_video: 'Видео',
        type_audio: 'Аудио',
        type_image: 'Изображение',
        model_any: 'Любая',
        date_from: 'С даты',
        date_to: 'По дату',
        advanced_filters: 'Расширенная фильтрация',
        group: 'Группа',
        group_any: 'Любая',
        tab_any: 'Любая',
        merge_loras: 'объединить LoRA',
        lyrics: 'слова',
        lyrics_strength: 'сила слов',
        vocal_volume: 'громкость вокала',
        audio_len: 'длительность аудио-сэмпла',
        top_k: 'топ K',
        diff_temp: 'температура диффузии',
        style: 'стиль',
        abc_planning: 'планирование ABC',
        abc: 'нотация ABC',
        abc_notation: 'сгенерированная ABC',
        abc_send_to_field: 'отправить в поле ABC',
        abc_send_to_tab: 'отправить во вкладку YuE2',
        abc_empty: 'нотации ABC пока нет',
        abc_mode: 'режим ABC',
        music_mode: 'режим музыки',
        mode_full: 'Полный (мелодия + аккорды)',
        mode_melody: 'Только мелодия',
        repetition_penalty: 'штраф за повторы',
        multiplier: 'множитель',
        interpolation_multiplier: 'множитель уплавнения',
        size_mp: 'размер в мегапикселах',
        megapixels: 'размер в мегапикселах',
        custom_output_resolution: 'кастомное выходное разрешение',
        reference_resolution: 'референс-разрешение',
        noise_scale: 'шкала шума',
        audio_neg_prompt: 'негативный аудио-промпт',
        audio_cfg: 'аудио CFG',
        start_step: 'первый шаг',
        end_step: 'последний шаг',
        speech: 'речь',
        version: 'версия',
        references: 'Референсы',
        reference_images: 'изображения-образцы',
        ref_images: 'изображения-референсы',
        ref_videos: 'видео-референсы',
        ref_audio: 'аудио-референсы',
        ref_image_size: 'размер референса',
        ref_megapixels: 'целевые мегапиксели',
        scale_match: 'подогнать',
        scale_max: 'макс',
        scale_scale: 'масштаб',
        upsample_spatial: 'два прохода',
        upsample_temporal: 'удвоить FPS',
        upsample_audio: 'улучшить аудио',
        upsample_sampler: 'сэмплер для улучшения',
        upsample_scheduler: 'планировщик для улучшения',
        distill_strength: 'вес ускоряющей лоры',
        upsample_distill_strength: 'вес ускоряющей лоры',
        upsample_detail_strength: 'вес детализирующей лоры',
        keyframes: 'ключевые кадры',
        keyframe_position: 'позиция кадра',
        keyframe: 'ключевой кадр',
        keyframe_strength: 'сила кадра',
        reference_audio_enabled: 'аудио-референс',
        trim: 'обрезать аудио (сек)',
        text_encoder: 'модель кодирования текста',
        chat_title: 'Чат',
        chat_placeholder: 'Введите текст для улучшения промпта...',
        chat_send: 'Отправить',
        chat_avatar_user: 'Пользователь',
        chat_avatar_assistant: 'Ассистент',
        chat_interrupt: 'Прервать',
        chat_new_chat: 'Новый чат',
        chat_refine: 'Уточнить',
        chat_clear: 'Очистить',
        chat_edit: 'Редактировать',
        chat_refine_confirm: 'Уточнить промпт',
        chat_refine_confirm_content:
            'Текущий чат будет очищен, а текущий промпт подставлен в поле ввода чата для уточнения.',
        chat_placeholder_right: 'Чат с AI для подсказок',
        chat_mode: 'Режим чата',
        yue2_chat_mode_generation: 'Генерация',
        yue2_chat_mode_editing: 'Редактирование',
        send_to_prompt: 'Отправить в промпт',
        regenerate: 'Перегенерировать',
        prompt_duplicate: 'повторить промпт',
        source: 'аудио в качестве источника',
        igc: 'коэффициент усиления идентичности',
        keep_size: 'оригинальный размер',
        'loop.enabled': 'зациклить',
        'loop.overlap': 'перекрытие кадров',
        'loop.strip': 'отрезать первые кадры (цикл будет лучше)',
        keyframe_trim: 'обрезать видео',
        keyframe_last: 'с конца',
        ref_video_trim: 'обрезка видео',
        ref_video_last: 'с конца',
        step: 'шаг',
        refmods: 'референс-моды',
        mod_name: 'имя мода',
        mode: 'режим',
        optimize_steps: 'шаги оптимизации',
        ref_resolution: 'референс-разрешение',
        latent_frames: 'латентные кадры',
        merge: 'слияние',
        motion_only: 'только движение',
        max_tokens: 'макс. токены',
        audio_max_seconds: 'макс. сек. аудио',
        audio_max_tokens: 'макс. токены аудио',
        i2i: 'image-to-image',
        videos: 'видео',
        correct_brightness: 'коррекция яркости',
        overlap: 'перекрытие',
    },
    settings: {
        select_language: 'Выбор языка',
        save_history: 'Сохранять историю локально',
        save_outputs_locally: 'Сохранять результаты локально',
        disable_help: 'Отключить подсказки',
        enable_previews: 'Включить предпросмотр',
        tag_completion: 'Дополнение тегов',
        backup_uploads: 'Сохранять/восстанавливать загруженные файлы',
        tag_number: 'Количество предлагаемых тегов',
        tag_enabled_tabs: 'Теги разрешены на вкладках',
        activation_tags_enabled_tabs: 'Activation tags разрешены на вкладках',
        chat_stream: 'Стриминг чата',
        letterbox_crop: 'Letterbox для кропа рефмодов (чёрные полосы по одной оси)',
        llm_url: 'LLM URL',
        llm_api_key: 'API key LLM',
        llm_model: 'Модель LLM',
        llm_supports_video: 'Поддержка видео LLM',
        llm_supports_audio: 'Поддержка аудио LLM',
        override_auto: 'Авто (из конфига)',
        override_on: 'Включено',
        override_off: 'Выключено',
        notification: 'Звук уведомления',
        notification_name: 'Звуковой эффект №%{number}',
        notification_none: 'Нет',
        unit: 'Единица',
        newer: 'Новее',
        older: 'Старее',
        clear: 'Удалить %{cmp}',
        seconds: 'секунда |||| секунды |||| секунд',
        minutes: 'минута |||| минуты |||| минут',
        hours: 'час |||| часа |||| часов',
        days: 'день |||| дня |||| дней',
        weeks: 'неделя |||| недели |||| недель',
        months: 'месяц |||| месяца |||| месяцев',
        years: 'год |||| года |||| лет',
        nothing_to_delete: 'Нечего удалять',
        nothing_to_delete_text:
            'Нет результатов %{cmp}, чем %{number} %{unit}.',
        clear_history: 'Очистка истории',
        tab_order_reset: 'Сбросить порядок вкладок',
        tab_order_reset_text: 'Порядок вкладок будет сброшен к исходному.',
        tab_order_reset_ok: 'Сбросить',
        clear_history_text:
            'Вы уверены, что хотите удалить результаты %{cmp}, чем %{number} %{unit}?<br />%{toDelete}',
        to_delete:
            '<b>%{smart_count} результат<b> будет удалён. |||| <b>%{smart_count} результата</b> будут удалены. |||| <b>%{smart_count} результатов</b> будут удалены.',
        prompt_active:
            'Фильтр активен! Будут удалены объекты, подходящие под "%{prompt}"',
        pinned_active: 'Только закреплённые объекты будут удалены!',
        type_active: 'Только объекты с типом %{type} будут удалены!',
        version: 'версия: %{version}',
    },
    help: {
        sampler:
            'Сэмплер управляет удалением шума с изображения с помощью различных алгоритмов. Некоторые сэмплеры более корректны и создают больше деталей, но они также медленнее. Экспериментируйте.',
        scheduler:
            'Планировщик определяет распределение шагов во времени. Обычно модель обучается на 1000 шагах, но мы используем только 10-50. Планировщик решает, какие именно шаги выбрать из этих 1000. Может улучшить или ухудшить качество и создать артефакты.',
        flow_shift:
            'Увеличьте для стабилизации движения при малом числе шагов (<20)',
        tea_cache:
            'Определяет, переиспользовать ли результаты предыдущего шага. Если выход первого слоя изменился менее чем на N по сравнению с предыдущим шагом (0.1 или 10% по умолчанию), то вместо полного вычисления используется выход последнего слоя с предыдущего шага. Если вы замечаете плавающий фон, который движется вслед за другими движениями, уменьшите этот параметр.',
        wave_speed_maxhit:
            'Столько шагов подряд могут использовать трюк с кэшированием, после чего будет форсировано полное вычисление. 0 отключает кэширование (каждый шаг будет полностью просчитан), -1 разрешает кэшировать неограниченное число шагов подряд. Если вы замечаете плавающий фон, который движется вслед за другими движениями, уменьшите этот параметр. Для ускорения рендера задайте большее число или -1.',
        tea_cache_start:
            'Кэширование активируется с этого шага (задаётся как доля от общего числа шагов). Увеличьте значение, если видео содержит артефакты движения, чтобы больше шагов в начале генерации было рассчитано полностью, без ускорения. Ранние шаги определяют общую структуру видео, а поздние шаги улучшают мелкие детали.',
        tea_cache_end:
            'Кэширование перестанет работать после этого шага (задаётся как доля от общего числа шагов). Уменьшите значение, если вам нужно больше деталей. Ранние шаги определяют общую структуру видео, а поздние шаги улучшают мелкие детали.',
        guidance:
            'Вес промпта, если результат недостаточно соответствует описанию, или если вы замечаете пропадающие конечности и объекты, увеличьте это значение.',
        seed: 'Сид — это число репродуцирования. Чтобы получить новый результат при неизменных остальных параметрах, поменяйте сид. Сиды не связаны друг с другом, так что изменение даже на 1 даст непредсказуемо отличающийся результат. Но один и тот же сид всегда выдаёт один и тот же результат (если не менять остальные параметры).',
        lora: 'LoRA (low-rank adaptation) — это метод добавления новых концептов в модель, таких как персонажи (люди), движения, жесты, стили и т.п. Вы можете комбинировать несколько лор с разной силой. Если вы замечаете странные артефакты, попробуйте уменьшить силу лор, особенно, если вы используете несколько одновременно.',
        virtual_vram:
            'Виртуальная VRAM позволяет выгружать слои модели в обычную память и загружать заранее, по мере необходимости. Скорость остаётся практически неизменной, но при этом появляется возможность создавать более длинные видео более высокого разрешеия. Работает только с GGUF.',
        riflex: 'RIFLEx позволяет создавать длинные видео (>5 секунд) без зацикливания. Если вам нужно зацикленное видео, задайте максимально доступную длину и выключите RIFLEx.',
        upscale:
            'Также сохранить результат как анимированный WEBP без потери качества, позволяет отправлять видео во вкладку Postprocessing для дальнейших улучшений.',
        nag: 'Normalized Attention Guidance позволяет использовать негативный промпт в гайданс-дистиллированных моделях (или оптимизированных моделях с CFG=1).',
        nag_scale: 'Сила негативного промпта.',
        nag_alpha:
            'Фактор смешения между позитивным и негативным промптом, бо́льшие значения означают бо́льшую силу негативного промпта.',
        nag_tau:
            'Ограничивающий фактор, чтобы предотвратить слишком сильный вылет негативного промпта.',
        nag_sigma_end:
            'Перестать применять NAG на этой сигме, даёт ускорение за счёт некоторой потери качетсва. Можно оставить на 0.9.',
        audio_scale:
            'Увеличьте значение, если персонаж недостаточно хорошо следует речи.',
        audio_cfg_scale:
            'Даёт эффект только если используется основной CFG. Выполняет два прохода за шаг, что потенциально может улучшить движение и детализацию за счёт значительно более медленного рендера.',
        mode_help:
            'Как референс кодируется в мод. «Полный» кодирует его целиком — все исходные кадры (до лимита) в целевом разрешении: максимальное качество, больше токенов. «Сжатый» пулит его в меньшую латентную сетку: заметно меньше токенов и деталей; также открывает опции слияния / только движение / шаги оптимизации ниже.',
        ref_resolution_help:
            'Целевое короткое ребро в px (только уменьшение, никогда не увеличивает). «Полный» сохраняет на этом разрешении; «Сжатый» кодирует с меньшим. 512 вдвое дешевле; 2048 — максимум детализации (x4 токенов от 1024).',
        latent_frames_help:
            'Временный лимит на видео. «Полный» сэмплирует до этого числа кадров источника перед кодированием; «Сжатый» пулит до этого числа латентных кадров. Ставьте не меньше числа кадров источника, чтобы избежать сэмплинга. Для картинок = 1.',
        merge_help:
            'Только сжатый режим. Вместо стекания каждого рефа совместно уточняет ОДНУ общую сетку против всех полных энкодов — мод совпадает с общим для всех видов (один крошечный консенсусный мод).',
        motion_only_help:
            'Экспериментально. Видео-рефы превращаются в покадровые временные разности, так что мод несёт где/как объекты движутся, а не их вид. Только сжатый режим; статичные кадры сохраняют вид.',
        multiplier_help:
            'Повторить извлечённый реф N раз по времени, чтобы короткий клип/гиф не затерялся в токенах основного видео. 1 = без повтора; размер файла растёт с N.',
        max_tokens_help:
            'Жёсткий лимит на общее число токенов, которые вставляет мод (0 = без лимита; 5120 — хороший дефолт). Если рефов больше, сначала отбрасываются почти-дубликаты латентных кадров, затем кадры ресемплируются под лимит.',
        crop_images_help:
            'Обрезает все референс-изображения к одному целевому аспекту. Без этого сервер центрирует каждый реф по канве первого изображения — удобно для единообразия, но может обрезать смещённые детали, например лицо. Используйте это, чтобы выбрать аспект и сдвинуть каждый кроп так, чтобы важная часть осталась в кадре.',
        optimize_steps_help:
            'Только сжатый режим. Градиентные шаги, уточняющие пул-латент и снижающие ошибку реконструкции. 0 = только пулинг, без уточнения. Это не сила identity и не обучение H3 — мод всё равно несёт концепт/движение, а не тонкий identity. По умолчанию 500.',
        audio_max_seconds_help:
            'Максимальная длина аудиореференса в секундах; нода обрезает более длинные клипы.',
        audio_max_tokens_help:
            'Бюджет токенов для аудиореференса (0 = без лимита). Аудио VAE кодирует до этого числа токенов.',
    },
    presets: {
        title: 'Пресеты',
        empty: 'Пресетов пока нет',
        search: 'Поиск пресетов...',
        all_tabs: 'Все вкладки',
        save_current: 'Сохранить текущее как пресет',
        name: 'Название',
        new_preset: 'Новый пресет',
        media: 'Медиа',
        params: 'Параметры',
        toggle_media: 'Все медиа',
        toggle_params: 'Все параметры',
        total_size: 'всего %{size}',
        add_file: 'Добавить файл',
        replace_file: 'Заменить',
        missing_server: 'Файл не найден на сервере ComfyUI',
        no_local_copy: 'Нет локальной копии',
        collect_failed: 'Не удалось скачать файл',
        saved: 'Пресет сохранён',
        save: 'Сохранить',
        edit: 'Редактировать пресет',
        tab_label: 'Вкладка',
        apply: 'Применить',
        applied: 'Пресет применён',
        file_lost: 'Файлы не найдены: %{files}',
        overflow: '%{n} файл(а) не добавлено: достигнут лимит поля %{field}',
        undo: 'Отменить',
        delete_confirm:
            'Удалить пресет «%{name}» вместе с сохранёнными медиафайлами?',
    },
    sessions: {
        title: 'Сессии',
        empty: 'В этой вкладке нет сессий',
        saved_reset: 'Сессия сохранена, форма сброшена',
        file_collect_failed: 'Не удалось сохранить: %{file}',
        restore: 'Восстановить',
        restore_title: 'Восстановить сессию «%{name}»?',
        restore_delete: 'Восстановить + удалить',
        restored: 'Сессия восстановлена',
        rename: 'Переименовать сессию',
        delete_confirm:
            'Удалить сессию «%{name}» вместе с её медиафайлами?',
        file_lost: 'Файлы не найдены: %{files}',
    },
    arena: {
        title: 'Elo-арена',
        select: 'Выбрать участников',
        exit: 'Выйти из режима выбора',
        start: 'Запустить арену',
        start_min: 'Нужно минимум 2 участника, чтобы начать',
        resume: 'Продолжить голосование',
        continue: 'Продолжить голосование',
        finish: 'Завершить арену',
        progress: '%{done} / %{total} матчей',
        confidence: 'Уверенность',
        leader: 'Лидер: %{name}',
        ci: 'CI',
        a: 'A',
        b: 'B',
        choose: 'Выбрать: %{name}',
        vote_this: 'Голосовать за это',
        view_full: 'Полный размер',
        exhausted: 'Все пары исчерпаны',
        points: '%{n} очков',
        wl: '%{w} побед – %{l} поражений',
        missing: 'Исходный ассет удалён',
        standings_hint: '%{n} участников — нажмите для списка',
        sync: 'Синхронизация видео',
        sync_hint:
            'Держать оба видео на одной позиции воспроизведения (и синхронный play/pause), чтобы сравнивать один и тот же момент',
    },
    export: {
        confirm_title: 'Экспорт',
        confirm_count: 'Будет экспортировано: %{n}.',
        confirm_filtered:
            'Применён текущий фильтр — будет выгружена только отфильтрованная часть.',
        exported: 'Экспортировано: %{n}',
    },
    import: {
        imported: 'Импортировано: %{n}',
    },
    toasts: {
        error_saving_preset: 'Ошибка сохранения пресета: %{err}',
        disconnected: 'Отключен',
        connected: 'Подключен!',
        error_getting_config: 'Ошибка получения конфига: %{err}',
        error_obj_info: 'Ошибка получения информации об объектах: %{err}',
        objects_updated: 'Объекты обновлены',
        execution_skipped: 'Выполнение пропущено',
        error_processing_handler:
            'Ошибка выполнения обработчика %{name}: %{err}',
        reloaded_objects: 'Объекты перезагружены',
        no_prev_result: 'Предыдущий результат не найден',
        no_base: 'Не выбран базовый результат для сравнения',
        no_params: 'Параметры генерации не сохранены',
        params_restored: 'Параметры генерации восстановлены',
        error_saving_history: 'Ошибка сохранения истории: %{err}',
        error_importing_database: 'Ошибка при импорте истории: %{err}',
        error_exporting_database: 'Ошибка при экспорте истории: %{err}',
        error_exporting: 'Ошибка экспорта: %{err}',
        error_importing: 'Ошибка импорта: %{err}',
        import_domain_mismatch: 'Архив для «%{got}», не для «%{want}»',
        error_uploading: 'Ошибка при загрузке изображения: %{err}',
        array_overflow:
            'Нельзя добавить больше изображений. Удалите или сбросьте одно из существующих.',
        folders_not_supported:
            'Папки не поддерживаются. Перетащите отдельные файлы.',
    },
    status: {
        [statusEnum.WAITING]: 'Ожидание...',
        [statusEnum.RUNNING]: 'В процессе',
        [statusEnum.FINISHED]: 'Завершено',
        [statusEnum.ERROR]: 'Ошибка',
        [statusEnum.INTERRUPTED]: 'Прервано',
        [statusEnum.CANCELLED]: 'Отменено',
    },
    refmods: {
        select_mod: 'Выбрать референс-мод',
        add: 'Добавить',
        strength: 'Сила',
        copies: 'Копии',
        search: 'Поиск мод...',
        all_kinds: 'Все типы',
        video: 'Видео',
        image: 'Изображение',
        audio: 'Аудио',
        video_n: 'Видео %{n}',
        no_audio: 'Без аудио',
        upload_audio: 'Загрузить аудио',
        no_mods: 'Референс-моды не найдены',
        delete: 'Удалить мод',
        create_panel: 'Создать референс-мод',
        mod_name: 'Имя мода (необязательно)',
        extraction_mode: 'Режим',
        compressed: 'Сжатый',
        full: 'Полный',
        delete_confirm: 'Удалить референс-мод "%{name}" вместе с его файлами?',
        import_result: 'Импортировать результат',
        import_failed: 'Не удалось импортировать референс-мод',
        edit_metadata: 'Редактировать метаданные мода',
        name: 'Имя',
        save: 'Сохранить',
        thumb_horizontal: 'Горизонтальное смещение',
        thumb_vertical: 'Вертикальное смещение',
        no_thumbnail: 'Нет миниатюры для этого мода',
        crop_images: 'Обрезать изображения',
        crop_title: 'Обрезка референс-изображений',
        crop_aspect: 'Итоговый аспект',
        crop_out: 'Размер кропа',
        crop_apply: 'Обрезать',
        crop_load_failed: 'Не удалось загрузить изображение',
        crop_failed: 'Не удалось обрезать изображение: %{err}',
        mod_resolution: 'Разрешение мода',
        over_budget: 'превышает лимит токенов',
        video_note: '+%{n} видео',
    },
    errors: {
        missing_controls: 'Отсутствующие контролы (есть в API): %{list}',
        missing_bindings:
            'Отсутствующие привязки к API (есть контролы): %{list}',
        missing_ids: 'Отсутствующие id узлов в API: %{list}',
        missing_fields: 'Отсутствующие поля в API: %{list}',
        interrupted: 'прервано',
    },
};

export default translation;
