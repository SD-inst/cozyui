...
A standalone <Picture N> line exists only when the picture itself is a frame anchor or storyboard reference; it states the anchor role and what the frame shows. The same picture may additionally be cited inside a <Subject N> definition when it also provides a subject's appearance, face, body, scene, costume, or style. A picture that only provides subject reference content and has no frame anchor role gets no standalone line and is not cited anywhere else: no "provides facial reference"-style lines, no summary mentions, no retention lines, no citations in detailed_description — use the <Subject N> label there instead.
...
<Subject N> — visible content abstracted from the reference assets that is reused or modified in the target video: people, animals, objects, scenes, backgrounds, clothing, props, interfaces, visual effects, styles, actions, expressions, or poses. It is a content unit actually used in the target video, not the source file itself. One subject may be defined by multiple assets; when it is, combine the sources and state what each asset provides. One reference asset may provide multiple subjects.
<Picture N> — a standalone line only when the image itself is a concrete target frame or shot-planning anchor: the first frame, keyframe, last frame, edited keyframe, or composition anchor of a shot, or a storyboard reference (state which shots it maps to and what planning information it provides). The line states the anchor role and what the frame shows; the same image may also be cited inside a <Subject N> definition when it provides subject content. If an image only provides a subject's appearance, face, body, scene, costume, or style and has no frame anchor role, do NOT create a standalone picture line for it and do NOT write lines like "<Picture N> provides facial reference for ..."; cite it inside the <Subject N> definition that uses it.
...
One line for each label that has its own definition line in subject_definitions, preserving the meaning established there. Do not write (Sx) IDs here. A picture without a standalone line (cited only inside a <Subject N> definition) gets no line here.
