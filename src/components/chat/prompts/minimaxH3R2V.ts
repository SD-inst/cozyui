...
<Subject N> — visible content abstracted from the reference assets that is reused or modified in the target video: people, animals, objects, scenes, backgrounds, clothing, props, interfaces, styles, actions, expressions, or poses. One subject may be defined by multiple assets; when it is, combine the sources and state what each asset provides.
<Picture N> — a reference image used as a concrete target frame or shot-planning anchor: the first frame, keyframe, last frame, edited keyframe, or composition anchor of a shot, or a storyboard reference. If an image only defines a character, scene, costume, or style, do not create a standalone picture line; cite the image inside the corresponding <Subject N> definition instead.
...
<Audio N> — a standalone audio file or an audio track of a reference video that is copied or referenced: a copied signal, a background-music style, a voice timbre and delivery, or dialogue, lyrics, or sound effects from the source audio. If an <Audio N> corresponds to a target speaker, reuse that speaker's (Sx) ID in the definition.
A label keeps the same meaning in every later section.

SECTION: summary
One short English paragraph beginning with a square-bracketed task-type prefix, then the target video and its main reference relationships, using only the labels defined above. Never introduce a new label here.
...
The main body. Begin with one or two English sentences establishing the overall style, then describe visuals, actions, sound, and dialogue shot by shot in target-video playback order, inserting reference labels where they apply. For generation tasks write 350-500 English words; dialogue-dense content prioritizes fitting the complete spoken timeline rather than mechanically reaching a word count, and video-editing descriptions scale with the complexity of the source video.
[Shot 1] marks the opening shot and has no timestamp. Later shots use [Shot 2] At MM:SS.mmm, ... to mark cut times that are strictly increasing and fall within the duration N.
At the first clear appearance of an important <Subject N>, describe its referenced characteristics, its position in the frame, and its current action; continue using the same label in later shots without redefining it. Use natural phrasing for concrete frame anchors: "the shot begins from <Picture 1>", "the shot's keyframe corresponds to <Picture 2>", "the shot ends on <Picture 3>". When editing or continuing an original video, cite <Video N> where its source state, structure, or continuation relationship applies. Cite <Audio N> in the shot or semantic phase where the audio relationship is active and state whether the signal is copied or referenced.
...
When a referenced subject speaks, keep both the visual reference label and the speaker ID: <Subject 2> (S1) turns toward the woman and says: "[English] Last summer, I went to my grandfather's house." If the same subject speaks off-screen, keep the same form and mark it as off-screen. If the speaker does not correspond to a defined subject, use a stable voice description followed by (Sx).
Always write spoken and sung content in English double quotation marks. Inside the quotation marks include only the language tag and the actual user-provided spoken content, preserving every original word and punctuation mark verbatim, without translation or rewrite. When dialogue, narration, or lyrics from reference audio are directly reused, preserve the exact source words and original language; write [unclear] for unintelligible spans instead of guessing or paraphrasing them. When only timbre, rhythm, emotion, or delivery is referenced, do not carry the original dialogue from the reference audio into the target video.
...
length=8
aspect=16:9
description=The woman from video 1 eats a cookie in the coffee shop from picture 1. The man from video 2 enters with the dog from pictures 2 to 4 and the dog lunges at the cookie. Three quick shots. Use the voice from audio file 1 for the woman's lines.
